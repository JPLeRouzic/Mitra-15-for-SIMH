...
t_stat ptr_rd(uint16 e_reg, uint16 *val);
...
*/
t_stat ptr_dio_handler(uint16 inst, t_bool is_write) {
    switch(cpu_state.reg_E) {
    	case 0x08:
	    if(is_write) {
		return ptr_wd(cpu_state.reg_E, cpu_state.reg_A);
		}
	    else {
		 return ptr_rd(cpu_state.reg_E, &cpu_state.reg_A);
		 }
	    break;
    
    	case 0x18:
	    if(is_write) {
		return ptp_wd(cpu_state.reg_E, cpu_state.reg_A);
		}
	    else {
		 return ptp_rd(cpu_state.reg_E, &cpu_state.reg_A);
		 }
	    break;
...
t_stat ptr_rd(uint16 e_reg, uint16 *result)
{
    ...
    *result = ((ptr.status & 0x03) << 13) | (ptr.last_char && 0x0FF);
    ptr.status = 0;
    return SCPE_OK;
}
