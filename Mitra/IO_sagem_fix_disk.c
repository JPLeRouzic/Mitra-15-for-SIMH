...
static void sagem_start_transfer(uint8 unitnb, uint32 mem_addr, uint32 byte_count, uint32 track, uint32 sector, int mode, int zio)
{
    ...
    if (sec >= sagem_state[unitnb].total_sectors) {
        sagem_state[unitnb].me = 0x0010;   /* bit 11 invalid sector address */
        sagem_state[unitnb].status = 0x01;
        sagem_interrupt();
        return;
    }
    if (byte_count == 0) {
        sagem_state[unitnb].me = 0x0080;   /* bit 12 zero word count */
...
*/
t_stat sagem_dio_handler(uint16 inst, t_bool is_write) {
    if(is_write) {
	return sagem_wd(cpu_state.reg_E, cpu_state.reg_A);
	}
    else {
	 return sagem_rd(cpu_state.reg_E, &cpu_state.reg_A);
	 }
}
...
t_stat sagem_wd(uint16 e_reg, uint16 a_val)
{
    ...
    if (cm == 0) {
        sagem_state[unitnb].me = 0x0008;   /* CN zero word count (bit12) */
        sagem_state[unitnb].status = 0x02;
        sagem_interrupt();
        return SCPE_OK;
    }
