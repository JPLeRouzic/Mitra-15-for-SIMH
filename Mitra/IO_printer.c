...
t_stat printer_rd(uint16 e_reg, uint16 *val);
t_stat printer_wd(uint16 e_reg, uint16 result);
...
*/
t_stat printer_dio_handler(uint16 inst, t_bool is_write) {
    if(is_write) {
	return printer_wd(cpu_state.reg_E, cpu_state.reg_A);
	}
    else {
	 return printer_rd(cpu_state.reg_E, &cpu_state.reg_A);
	 }
}
